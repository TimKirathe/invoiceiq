...
async def initiate_stk_push(
    ...
    if payment_method == "PAYBILL":
        ...
        account_reference = invoice.get("mpesa_account_number", invoice["id"][:12])
    else:
        # For TILL (or fallback): use invoice ID
        account_reference = invoice["id"][:12]

    # Ensure account_reference is max 12 characters (M-PESA limit)
    account_reference = account_reference[:12]
    transaction_desc = invoice["description"][:13]  # Max 13 characters (M-PESA limit)
